...
| Receipt-Capture-Harness (`/receipt`) | `_receipt_capture_checkpoint` | capture → confirm (or corrections) | Gemini Vision; GEMINI_API_KEY required; WORKSPACE_INTERNAL minimum; 60s vision timeout; TaxEvent tag tax:expense; photo uploaded to /Digiant/Tax/FY{year}/ post-confirm (non-blocking) |
...
| `/receipt` | Receipt-Capture-Harness | Send photo with /receipt caption → Gemini Vision extracts fields → Director confirms → TaxEvent written to semantic memory |
