...
13. **Every code review of existing code must include a YAGNI pass.** Apply this checklist to all existing code touched during a session, in order:
    1. Does this need to exist? → no: delete it (YAGNI)
    ...
    Report findings as: `file:Lnn: <tag> <what>. <replacement>.` where tag is `delete:` / `stdlib:` / `native:` / `yagni:` / `shrink:`. End with `net: -N lines possible`. If nothing to cut: `Lean already.`

14. **Crowd-edited / user-generated sources (Wikipedia, Reddit, and similar) are never a standalone authoritative source (2026-07-06).** Treat as a starting pointer at best. Applies with particular weight to anything contested or political — no editorial accountability means structural exposure to capture and edit-war instability on exactly the topics where accuracy matters most. Confidence max 0.6 ("single source") for controversial/political claims sourced from this category regardless of how confidently worded; require an independent corroborating source outside the category before raising confidence. Implemented in `personas/research_agent.md`'s epistemic standards, enforced in practice by the existing `_contrarian_gate` (`execution/engine.py` ~line 2765) which already requires independent interrogation of consensus, not an echo of it.
...
### GPU (EVGA RTX 3090, 24GB VRAM — power-capped at 350W)
...
- 350W cap via `/etc/systemd/system/nvidia-power-limit.service` — 420W caused FULLCHIP_RESET (daisy-chained 2×8-pin cable voltage sag 2026-06-19); raise to 420W when cables replaced
...
### `_quick_classify` design principles
- **Keyword guards fire only on Director messages, never on assistant content.** `_infer_prior_domain` reads user turns only — assistant responses discussing system domains must never poison routing. Rex legitimately mentions skills/docker/email in many answers; those mentions must not set prior domain context.
- **Shortcuts are for unambiguous system operations only.** Conversational language (e.g. "what's happening", "let me know when it") must fall through to PASS 1 LLM even if it contains system-adjacent words. Add a shortcut only when the phrase is unambiguous in isolation.
- **Shortcuts must not fire on system-domain context they are not about.** Broad shortcuts (e.g. `_research_live_kw`) must have a system-domain exclusion guard so "look into recent emails" does not route to web_search.
- **When in doubt, fall through.** Qwen3-32B handles routing correctly for ambiguous cases. The cost of a bad shortcut (misrouting, confabulation, poisoned context) is higher than the cost of one extra PASS 1 inference call.
...
### Cognition Engine — Subject scope, principle (2026-07-03)
- **Prefer narrow, single-question Subjects over broad multi-topic ones.** A Subject's thesis should answer one question, not several stitched together. `crypto` (market direction) / `crypto_revenue` (yield/earnings optimization) / `crypto_tech` (validator hardware/software/ops) is the reference split — not one broad "crypto" Subject trying to cover all three.
- **Why this matters, not just style**: every scoring/thought/research call against a Subject sends its *entire* thesis + open_questions + knowns as context — a conflated thesis is a bigger prompt on every call, AND produces worse relevance judgments, since the LLM has to decide "is this relevant" against a question that's secretly two or three different questions. Narrow subjects are cheaper AND more precise; this is not purely a cost/quality tradeoff where you can pick either side and be fine — the broad-thesis path loses on both axes at once.
- **Concrete failure mode observed**: `ai`'s thesis mixed "frontier AI industry watch" with "hardware/software for improving the Sovereign stack" (added via a direct Director note edit, no code review). Result: a RocketPool staking-commission story — nothing to do with AI — scored as relevant to `ai` during a `/learn` fold-in test, because the conflated thesis gave the scorer no clean signal to reject it against. Split into `ai` (industry) + `ai_ops` (Sovereign's own stack) to fix.
- **More subjects is the cheaper failure mode than broader ones.** N narrow Subjects costs one more scoring call per event (linear, predictable, pure latency — acceptable per the local-first design philosophy). A broad Subject costs bigger prompts everywhere it's used *and* degrades precision. When genuinely unsure whether a new topic warrants its own Subject or belongs inside an existing one's thesis, default to a new Subject.
- **Naming convention: `{parent}_{focus}`.** When a broad Subject splits into narrower ones, keep the parent's name as a prefix and add a clarifying suffix for what's different — `crypto` → `crypto_revenue` / `crypto_tech`; `ai` → `ai_ops`. Not `sovereign_ops` (loses the `ai` lineage), not a totally new unrelated name. The name alone should tell you which broader domain a narrow Subject sits under.
- **Where this applies**: `cognition/subjects.py`'s `create_subject()` is the single creation path (standing order #2 — no duplicate creation logic elsewhere). Apply this principle whenever a new Subject is proposed, whether Director-initiated or (if built later) autonomously proposed by Rex.

### Cognition Engine — current operational model (redesigned 2026-07-06)

A Subject (`semantic:subject:<id>`) updates via a **Thought** (`cognition/thoughts.py` `run_thought()`) — a bounded, goal-seeking research effort, max 3 iterations, triggered by RSS, web search, email, portfolio findings, or conversation. **No Director approval gate on the outcome** — `resolve_thought_outcome()` (`cognition/subjects.py`) decides and acts in one call. The confidence/quality model was substantially redesigned 2026-07-06 (the 2026-07-03 three-axis-gate + self-tightening model below is superseded, not additive):

- **Confidence moves by stance-weighted delta, not averaging.** `apply_confidence_delta()` steps confidence (bounded `_MAX_CONFIDENCE_STEP` 0.15, floor 0.05/ceiling 0.95) in the direction of the Thought's `thesis_stance` (supports/contradicts/neutral) × `stance_strength` × source-quality weight (`label_to_weight()`, was `confidence_to_score()` — now honestly scoped as a per-Thought trust weight, not the thesis confidence itself). Neutral stance or zero strength → delta 0. Replaces the deleted `rolling_confidence()`.
- **The only gate on whether a finding is applied is novelty** (`assess_thought_quality()`'s `novelty_score >= _NOVELTY_THRESHOLD`, 0.3). `evaluate_thought_iteration()` uses a separate fixed `_ITERATION_QUALITY_BAR` (0.50, per-iteration "did this search find enough to stop digging") — fully decoupled from the Subject's own `confidence_target`, which is a materiality/decision threshold, never an iteration-control bar.
- **Self-tightening/decay-on-miss mechanism deleted outright** (was: 2 consecutive misses → `confidence_target` +0.05). Confidence is now a genuine accumulator with its own ceiling, so the unreachability problem that mechanism compensated for doesn't arise the same way.
- **Popperian epistemic-status layer** — `get_epistemic_status(subject)` returns `"investigating"` (default) | `"corroborated"` | `"refuted"`, distinct from `status` (active-rotation membership). `evidence_weight` (`{"supports","contradicts","neutral"}` counters) tracks how many independent Thoughts actually weighed in, separate from confidence itself. A `questions_closed` entry with `stance: "contradicts"` → **refuted** (hard trigger, asymmetric — one is enough regardless of other supporting evidence); no open_questions + confidence≥target + no risk_flags + evidence_weight≥`_MIN_EVIDENCE_WEIGHT` (3) → **corroborated**. Both transitions are always-sent Telegram alerts (not urgency-gated) and call `propose_successor_thesis()` (one LLM call, drafts `thesis_components.falsification_conditions` first, derives `open_questions` 1:1 as closed yes/no operationalizations of them — "exist to be refuted, not proven"; writes a `subject_proposal` to PROSPECTIVE, Director-confirmed via the existing "approve `<phrase>`" path, never auto-created). `run_thought()` refuses to run further automatic Thoughts on a `refuted` Subject until the Director acts.
- **`succeeds`** — lineage field (frozen at creation) threading parent→child Subject generations when a successor thesis is created from a refuted/corroborated one.
- **Subject retrofit precedent (2026-07-07)**: pre-existing Subjects created before this redesign have `thesis_components: null`. Retrofitting one (done for `crypto_tech`, `crypto_revenue`, `macro` 2026-07-07) means: keep `thesis`/confidence/history untouched, add `thesis_components` (claim/mechanism/time_horizon/materiality/falsification_conditions) + reframe `open_questions` as closed yes/no 1:1 with the conditions, clean any scope-contaminated `knowns`, patch both the Qdrant point (`set_payload`, no re-embed needed for metadata-only fields) and the Nextcloud note in the same pass. No code changes required — pure content work.
- **Cross-subject reconciliation contamination (found 2026-07-07, not yet fixed)** — `learn subject <id>`'s reconciliation pass runs the note's full content through `_run_cognition_engine_pass()`, which triages against ALL active Subjects, not just the one being reconciled. Topically-adjacent Subjects (e.g. `macro`/`macro_demographic`/`retirement`, all touching interest rates) can end up citing each other's own prior output as "new evidence," inflating confidence via a feedback loop rather than genuine independent corroboration. Flagged, not fixed — candidate fix would need to detect when triaged text originates from another Subject's own note/reconciliation rather than a genuine external source.
- **Crash-recovery** — `run_thought()` writes a durable marker to `meta` (`meta:thought_inflight:{thought_id}`) at start, cleared at completion; `sweep_orphaned_thoughts()` runs once at boot and reports anything still marked in-flight from a prior restart.
- **Email is a real trigger, not just prioritization** — `run_score_email_by_subject()` calls `observe_for_subject()` for every non-phishing Subject match. See "Proactive Mailbox Management" below for the full current shape of this harness.
- **Scheduled-task activation**: "Reply approve X to activate" is handled by the `task_activate` intent + `find_pending_task_by_phrase()`/`activate_task()` in `task_scheduler.py`.

Full history and every intermediate design decision: `docs/cognition-engine-as-built.md` (RAID), `docs/as-built.md`'s 2026-07-03 through 2026-07-07 entries, and the durable Claude Code memory at `project_cognition_engine.md`.

### Proactive Mailbox Management (EA behavior) — current operational model (2026-07-07)

`run_score_email_by_subject()` (`monitoring/cognition_harness.py`) is the one harness behind both the Weekday Morning Briefing's business-account step and the standalone 15-min-cadence "Personal Mailbox Scan" task — as of 2026-07-07 that same 15-min task scans **both** personal and business (PROCEDURAL step `params.accounts: ["personal", "business"]`; business IMAP is fast, negligible added cost). Per email: `detect_brand_mismatch()` (phishing/spam) → `derive_urgency()` (high/medium/low) → `find_relevant_subjects()` (Cognition Engine triage) → `looks_like_calendar_candidate()` (cheap keyword pre-filter, gates `detect_calendar_event()`'s LLM call, capped at 5/run).

- **Spam auto-delete exception** (`cognition.spam_auto_delete`, default `true`) — a brand-mismatch flag bypasses `delete_email`'s normal HIGH-tier confirmation gate entirely; the classification itself is treated as the confirmation (Director: "I need to confirm spam so then it getting auto deleted is fine" — `detect_brand_mismatch()` is the one well-established, low-ambiguity spam signal in the system). Deletes via `nc-mail delete_message`, which moves to that account's Trash (recoverable within Nextcloud's Trash retention, not instant permanent erase). Falls back to flag-only if delete fails or the toggle is off.
- **Spam/urgent sender memory** — `record_spam_sender()`/`record_urgent_sender()` (`cognition/subjects.py`, shared `_record_flagged_sender()` helper) persist a deduped-by-sender-address record (`semantic:spam_sender:<addr>` / `semantic:urgent_sender:<addr>`, `count`/`recent_subjects`/`first_seen`/`last_seen`/`last_reason`). Before this, a flag only ever appeared once in a Telegram digest and left no trace.
- **Calendar-invite suggestions** — `email_event_suggest.py`'s `looks_like_calendar_candidate()` (config-driven keyword regex, `cognition.calendar_keywords`, same Director-editable pattern as `urgency_keywords`/`known_brands`) gates a body fetch + `detect_calendar_event()` LLM call. Suggests only, in the digest — never auto-creates the event.
- Every one of these knobs (`spam_auto_delete`, `calendar_keywords`, `urgency_keywords`, `priority_senders`, `known_brands`) lives in `sovereign-config.yaml`'s `cognition:` section — dashboard-visible/editable, not hardcoded, per the established pattern for anything with real-world consequences.
...
4. ~~**Soul guardian alert — governance.json mismatch**~~ — RESOLVED 2026-06-30. `record_baseline()` run; stored checksum updated to `fdbfbf58...` (v1.36 production file). Alert cleared.

5. **docker_logs self-container normalization (2026-06-29)** — LLM sets `target: "rex"` / `"sovereign"` / `""` when checking own logs → broker 403 (container not in allow_names). Fixed in `execution/engine.py` docker_logs dispatch: `_SELF_NAMES` set maps self-referential names → `"sovereign-core"`. Requires rebuild to take effect.

6. **query_distil timeout (pre-existing)** — `query_distil: timeout/error target=fast_llm` on every query. Distillation LLM call times out and falls back to raw query. Not introduced 2026-06-29.

7. **Tax harness — Rocket Pool discovery gaps (found during validator-monitor Phase B, 2026-07-01)** — five items surfaced live-probing node02, none blocking, all need a Director call before Phase E/F build on top of them:
   - **RPL untracked as an asset.** `pricing.py`/`classifier.py` have never handled RPL — only ETH/BTC. RP_NODE holds 58.59 RPL loose (confirmed on-chain via `balanceOf`, not at LEDGER_MINING or TREZOR_MINING) plus 3305.57 RPL staked as collateral. Needs an NZD price feed and a classification rule before RPL income/holdings can be reported.
   - **Restake-vs-withdraw tax timing is unresolved.** Director claimed Interval 48/49 rewards via `rocketpool node claim-rewards` and chose to restake the RPL portion rather than withdraw it (0.045318 ETH did land in LEDGER_MINING). Open question: does NZ tax law recognise the income at claim time regardless of restaking, or only on eventual sale/withdrawal? Classifier needs one consistent answer, not an assumption.
   - **Three distinct claimable-value buckets exist for Rocket Pool, not one.** (1) interval-based Smoothing Pool ETH + RPL via `claim-rewards`, (2) minipool refunds (CLI: "14 minipool(s) have refunds available!" — separate mechanism, bond-reduction related), (3) raw minipool contract balance pre-`distributeBalance()`. `ValidatorMonitorAdapter.get_pending_rewards()` (Phase C) needs to check all three.
   - **4 finalized/closed minipools not in the current 14.** `rocketpool node status` flagged these in passing — worth confirming those historical exits were already captured in the correct tax year rather than assuming.
   - **Rocket Pool node withdrawal confirmed manual, eth-docker confirmed automatic.** Confirmed on-chain: Rocket Pool → `RocketNodeManager.getNodeWithdrawalAddress()` = LEDGER_MINING (requires manual claim); eth-docker → beacon `withdrawal_credentials` = TREZOR_MINING (automatic sweep, already landing — TREZOR_MINING balance already reflects small periodic sweeps from the two near-32-ETH validators). Both destinations are already in `mining_wallets`, so Phase E's income-classification gap is narrower than originally scoped — the real gap is the manual-claim visibility (bucket 1 above), not a missing income category.
...
| Tax-Report-Harness | **COMPLETE** | /do_tax [year] command. 3-turn human-in-the-loop: query semantic (date range) → Director provides expense CSVs → confirm → generates income{year}.csv, expenses{year}.csv, and disposals{year}.csv in /Digiant/Tax/FY{year}/. Classifier labels crypto events. FIFO Phase 3 complete: full cross-year lot pool, on-the-fly CoinGecko/CryptoCompare enrichment, data_gaps→semantic:tax:data_gaps:{year}. |
...
| Research-Harness-Phase2 | **COMPLETE** | 6-agent adversarial security analysis pipeline in `research_harness.py`. Agents: News → Fundamentals → Sentiment → Bull → Bear → Risk Manager. `_classify_research_intent()` routes to security/financial_topic/general paths. `GatheredSources` dataclass. NZX dual-listing map. `_build_nzx_url()`. `security_analysis_engine()` callable from portfolio harness. `gateway/main.py`: `/research` command with 900s timeout + direct Telegram send. `run_portfolio_analysis()` now calls security engine per asset. End-to-end: RocketLab HOLD/HIGH, 1571 words, ~11 min. 2026-05-20. |
| ClawSec-Pipeline | **COMPLETE** | Automated ClawSec pattern refresh. `security/clawsec_harness.py`: `fetch_and_apply()` fetches GitHub raw feed → writes `clawsec_dynamic.yaml` + updates `.checksums.json` atomically. `scanner.py`: `_load_clawsec_dynamic()` loads dynamic YAML at startup. `soul_guardian.py`: `OPTIONAL_PROTECTED_FILES` — no alert before first run. `nanobot-01/security.py`: replaced hardcoded patterns with `NanobotSecurityModule` loading universal.yaml + clawsec_dynamic.yaml. `clawsec_update` intent (MID tier). SI harness `observe()`: step 2.6 checks pending/ dir → proposes clawsec_update if count>0 + oldest>7d. ncfs tier fix: fs_move/copy/mkdir/delete/tag/untag → MID. 2026-05-27. |
| Portfolio-Harness-Phase2 | **COMPLETE** **+ SCHEDULER TIMEOUT-LOOP FIX 2026-07-12** **+ CATEGORY-PARAM FIX 2026-08-02 (uncommitted)** | Correlation analyst (7th agent), stress tests (3 scenarios, asyncio.create_task post-notify), monthly analysis task (28th, 8am NZST, pending_approval seed), weekly watcher scan (RSI/MACD signals → full 6-agent on trigger, silence on clean week). `task_scheduler.seed_monthly_portfolio_task()` + `seed_weekly_watcher_task()` — idempotent at startup. `portfolio_watcher` domain added to governance + engine dispatch. `portfolio_watcher_scan` intent (LOW). 2026-06-01. **2026-07-12**: weekly scan looped 6x in 33min (duplicate Telegram spam, same 4 assets re-analyzed each cycle) — root cause was a generic `task_scheduler.py` bug (timeout cancels `_execute_task()` before it advances `next_due`, so scheduler re-fires from scratch every 60s poll) compounded by this task's 4-5-asset sequential scan routinely exceeding the 300s global timeout. Fixed scheduler-wide (`_advance_next_due_after_failure()` on timeout/exception) + gave this task a `execution_timeout_s: 900` override (data-driven, `store_task()`-persisted, no task-specific scheduler code). Also fixed an unrelated false "note save failed" warning in `portfolio_analysis_harness.py` found in the same investigation (checked `status` on the wrong unwrapped dict — notes were saving fine). **2026-08-02**: the 2026-07-31 monthly run failed silently on both steps (`Portfolio analysis: No ledger found for ''`) — `task_scheduler.py`'s `_execute_task()` merges step params into `action`, never `delegation`, but `execution/engine.py`'s `domain=="portfolio_analysis"` handler only read `category`/`last_day_guard` from `delegation` (always `None` on the scheduler path) — `category` silently resolved to `""` every month since the task was first wired (2026-06-01), never caught until this run. Fixed: `_pa_cat`/`last_day_guard` now also fall back to `action.get(...)`. Compounding bug also fixed: `_format_step_content()` missed `message`/`director_message` in its detail fallback chain and mis-flagged the legitimate `"running"` status as ❌. Uncommitted — needs a rebuild before 2026-08-31 to avoid recurrence. See `core/app/CLAUDE.md`'s Task Scheduler section + `docs/as-built.md` for full detail. |
| WorldMonitor-Integration | **CODE COMPLETE, UNCOMMITTED (2026-07-28)** | Two pilot domains from WorldMonitor (third-party intelligence aggregator on node04, loopback, pinned commit `4ffaa80d`) wired in as grounded, non-LLM-fabricated input. `a2a-browser`: new `POST /worldmonitor` proxy route (dumb pass-through, no interpretation) mapping `domain` → upstream `get-risk-scores`/`get-macro-signals` paths. Bespoke `worldmonitor` nanobot-01 skill (`/home/sovereign/skills/worldmonitor/SKILL.md`, checksummed, harness-internal only — no Director-facing NL trigger): owns the one real policy layer, a two-stage staleness gate (age-from-timestamp decides suppress-vs-surface, since the vendor's own `stale` flag was found live to under-report staleness; degraded/stale/unavailable flags decide assertable-vs-caveat-only only once past the age gate) and a `verdict`/`bullishCount` → `consensus_to_interrogate` relabel kept structurally separate from grounded `signals` throughout (anti-consensus obligation, `sovereign-soul.md`). `core/app/monitoring/worldmonitor_memory.py`: single canonical EPISODIC+META write path shared by both consumers (SDO #2/#3). Consumers: `news_harness.py` (`_fetch_worldmonitor_risk()` — third parallel news-brief source, no dedicated Subject yet) and `portfolio_analysis_harness.py` (`_gather_worldmonitor_macro()` — fetched once per run, threaded into every asset's synthesis prompt as two separate sections, grounded signals vs. third-party consensus, never merged). Needs `sovereign-core` + `a2a-browser` rebuild before live; not yet verified against a real WorldMonitor payload. Full detail: `docs/as-built.md`. |
| Nanobot-Dispatch-Queue | **CODE COMPLETE, UNCOMMITTED (2026-08-02)** | `adapters/nanobot_queue.py` — bounded-concurrency, per-skill-name priority queue for nanobot-01 dispatch, modeled on `InferenceQueue`'s shape but N-worker (not single-worker, since a2a-browser/IMAP/WebDAV can serve concurrent requests unlike the GPU) and one queue per skill (unrelated backends have unrelated real capacities — `sovereign-browser` capped at 4 from an observed 18-deep-burst empty-result incident, everything else defaults to 6). Timeout propagates as a raised `asyncio.TimeoutError` through the future rather than a resolved status dict, matching `nanobot.py _forward()`'s existing exception-based error convention (contrast `InferenceQueue`, whose callers expect a dict). `NanobotAdapter._get_queue()` creates queues lazily per skill on first use; `_forward()` submits through the queue with a wall-clock timeout backstop over httpx's own timeout, plus a >10s slow-call diagnostic log. `main.py` lifespan wires `nanobot.start()`/`.stop()` symmetrically with `inference_queue.stop()`. Related: `inference_queue.py` timeout logging now also captures Ollama's actual running models via `/api/ps`. Needs `sovereign-core` rebuild before live; not yet load-tested against a real concurrent burst. Full detail: `docs/as-built.md`. |
| Self-Interrogative-Routing | **COMPLETE** | Rex can answer identity and architecture queries. 3 new intents: (1) `memory_recall` for "who are you"/"your architecture" etc — reads `semantic:sovereign-self-architecture` (exclusion guard prevents tier/skill/collection queries matching). (2) `memory_list_collections` — static handler returns all 8 Qdrant collections. (3) `governance_describe` — reads governance.json deterministically, formats tier summary. `_quick_classify` fast-paths + `_system_signals` extensions for all 3. `governance/engine.py`: memory `recall`, `list_collections`, `governance_read` domain all added. `sovereign-self-architecture.md` at `/downloads/`. Commit 66bac80. 2026-06-02. |
| Quick-Classify-Refactor | **COMPLETE** | Full audit + 8 fixes across 2 sessions (2026-06-04 + 06-08). Removed: `_news_kw` over-broad patterns (latest news/news summary/recent news etc — 6 patterns), 4 over-broad `_sched_early` phrases, dead `_web_search_kw` tuple. Added: `_research_live_kw` system-domain exclusion guard (email/file/docker/calendar). Removed "do you remember"/"what do you remember" from `_mem_list_kw`. `_infer_prior_domain` now reads user turns only (prevents assistant response poisoning context). `_GENERAL_LOOKUP_SYS_DOMAINS` "skills" gate removed. `ask_conversational()` SOVEREIGN CAPABILITIES block added. `_NEGATION_RE` guard before `_XPROV_RE`. Design principles added to CLAUDE.md. |
| Query-Distillation | **COMPLETE** | Search quality overhaul. (1) `query_type` audit field on `InternalMessage` — `_derive_query_type()` maps intents to current_events/local_context/general_knowledge, injected at PASS 1. (2) Conversational guard routing fix — guard now fires even with `prior_has_system=True` unless domain-reference pronouns present; "best pizza in Manhattan" no longer hits SearXNG. (3) Conditional secondary fetch — when enrichment is poor (`confidence < 0.6`), top 2–3 URLs fetched in parallel; translator synthesises with inline citations. (4) PASS 1 diagnostic-phrase misrouting fix (8 new `_capability_kw` phrases). 2026-06-07. |
| Governance-Fixes-Jun08 | **COMPLETE** | 10 fixes across 2 sessions. `remember_fact` + `validator_queue_check` tier "NORMAL" → "LOW". `nanobot_run`/`nanobot_health` added to governance.json `allowed_actions`. `governance/engine.py`: `nanobot` + `validator_queue` domain handlers added. Confabulation guards added to `_general_lookup` + `ask_conversational`. `_infer_prior_domain` user-turns-only. "skills" gate removed from `_GENERAL_LOOKUP_SYS_DOMAINS`. "do you remember" removed from `_mem_list_kw`. `note_path` added to research semantic memory. `file_read` 404 fallback lists parent folder. 2026-06-08. |
| News-Routing-Fixes-Jun18 | **COMPLETE** | 5-fix chain for SpaceX IPO routing failure. Removed 6 over-broad `_news_kw` patterns. `_NEGATION_RE` guard before `_XPROV_RE`. SOVEREIGN CAPABILITIES block in `ask_conversational`. Conversational guard returns `None` (not `intent=query`) — lets PASS 1 classify. PASS 1 prompt OVERRIDE rule: live-data signal beats summarise verb. `content_synthesis` task type added — initially Gemini-first; changed to groq_inference-first 2026-06-29 (see Structured-Generation entry). `_FREE_FIRST_TASK_TYPES` frozenset. Governance.json v1.36 + EXTERNAL_COGNITION.md v1.2 (external-first doctrine). Commit d54a08d. 2026-06-13/18. |
| PASS4-Fallback-Fix | **COMPLETE** | When PASS 4 LLM produces malformed JSON, `_parse_llm_output` fell back to `{"success": False, "outcome": "Evaluation failed."}` — Director saw "That action failed: Evaluation failed." even when PASS 3b succeeded. Fix (`cognition/engine.py` ~line 1097-1112, moved from the original ~869 as the file has grown): defaults now populated from `specialist_inbound_result` fields — success/outcome/detail/error passed through instead of overriding with failure. Observed during `list_nc_tasks` 2026-06-17. 2026-06-18. |
| Pydantic-Schemas | **COMPLETE** | Pydantic v2 output schemas for all 5 cognitive passes (`cognition/schemas.py`): `Pass1Output`, `Pass2Output`, `Pass3aOutput`, `Pass3bOutput`, `Pass4Output`, `ResultForTranslator`. All `extra="allow"` — drift detection without rejecting unknown LLM fields. `_parse_llm_output()` wired with `schema=` param for PASS 3a/3b drift detection (never-raises). `Pass2Output.model_validate()` inline in `security_evaluate()`. 2026-06-29. |
| Structured-Generation | **COMPLETE** | `_llm_generate_structured(prompt, schema, defaults, priority, pass_name, max_retries=2)` — queue-aware retry transport in `cognition/engine.py`. Retry loop injects `ValidationError` field violations into follow-up prompt. `/no_think` on all attempts. Exhausted retries → return defaults + `llm_structured_exhausted` ledger entry. Wired to PASS 1 (`ceo_classify`) and PASS 4 (`orchestrator_evaluate`). `_rft_check` guard changed from `"outcome" not in` to `not _rft_check.get("outcome")` (model_dump fills `outcome=""`). `content_synthesis` task type changed from `gemini` → `groq_inference`. `groq==1.0.0` + `instructor[groq]==1.15.4` added to requirements.txt. 2026-06-29. |
| Chonkie-Chunker | **COMPLETE** | `scripts/seed_memory.py`: bespoke `chunk_text()` (21 lines) replaced with `TokenChunker(WordTokenizer(), chunk_size=200, chunk_overlap=0)`. Module-level `CHUNKER` constant. WordTokenizer uses `split(" ")` — counts run ~10% over target on dense markdown; well within nomic-embed-text 8192-token limit. 2026-06-29. |
| Validator-Skill | **COMPLETE** | Ethereum validator monitoring via local Nimbus beacon API. `GET /system/validators` + `GET /system/validators/sync` added to docker-broker (`/docker/sovereign/broker/index.js`). Config: `/docker/sovereign/broker/validators.json` (17 indices, node address, beacon URL). `BrokerAdapter.get_validators()` + `get_validator_sync()` in `adapters/broker.py`. `validator_list` + `validator_sync` intents in `INTENT_ACTION_MAP` (domain: docker) + `INTENT_TIER_MAP` (LOW). `_quick_classify` fast-path: validator keyword + status verb → routes before PASS 1 LLM. `_system_signals` extended with "validator", "minipool", "rocketpool" etc. Both endpoints added to `docker-policy.yaml` low-trust allowlist. Semantic memory entry `semantic:eth:validator-indices` written to qdrant-archive. 17 validators, all active_ongoing, ~544 ETH. 2026-06-21. |
| Validator-Monitor-Harness | **PHASES 0–D, F, G COMPLETE — E DEFERRED** | `execution/adapters/validator_monitor.py`: query-only, on-chain `eth_call` + beacon API reads, no Smartnode HTTP dependency (loopback-only, unreachable). 17 validators across two populations (14 Rocket Pool LEB8 minipools, 112 ETH bond + 3 eth-docker solo, 96 ETH bond = 208 ETH total). `check_validators`/`validator_alerts` intents (LOW, devops_agent). **Phase E deferred** — Director confirmed no further progress possible without querying node02 directly (Smartnode API loopback-only); 3-bucket claimable-value gap stands, see "Pending Director Decisions" item 7. **Phase F**: synthetic `eth_staked` `AssetSpec` injected live into `/portfolio` crypto resolve (`_inject_staking_position()`) — no double-counting (withdrawal-wallet balances already tracked separately), FIFO cost-basis pipeline reused generically, research/synthesis pipeline skipped via new `staking_position` short-circuit (mirrors `stablecoin` pattern). **Phase G**: `seed_daily_validator_task()` — daily 07:00 NZST `check_validators` run, `pending_approval`, mirrors monthly/weekly portfolio seed pattern; found+fixed a `notify_when: "on_findings"` gap (`_result_has_content()` didn't recognise the `alerts` key — would have been permanently silent). Validator set never pinned — both re-derive live every run, no re-seed needed across the Director's planned megapool migration. 2026-07-01. |
...
| Cognition-Engine | **COMPLETE**, redesigned 2026-07-06, Subject retrofits + EA mailbox 2026-07-07 | Subjects (`semantic:subject:<id>`) updated via Thoughts (max 3 iterations) triggered by RSS/web search/email/portfolio/conversation. Confidence/quality model fully redesigned 2026-07-06 — stance-weighted `apply_confidence_delta()` replaces averaging, self-tightening deleted, Popperian `epistemic_status` (investigating/corroborated/refuted) + `evidence_weight` + `propose_successor_thesis()` added (see "Cognition Engine — current operational model" above for the current shape; the 2026-07-03 three-axis-gate description is superseded). 2026-07-07: `crypto_tech`/`crypto_revenue`/`macro` retrofitted onto the `thesis_components` template, new `crypto_exit` Subject created; ETH/BTC ratio rotation watcher added to the portfolio harness; proactive mailbox management built out (business account added to the 15-min scan, calendar-invite suggestions, spam auto-delete exception, spam/urgent sender memory — see "Proactive Mailbox Management" above). `_synthesise_topic()`'s Telegram summary bug fixed (was echoing the full prompt/trigger back verbatim instead of the actual finding). Full detail: `docs/cognition-engine-as-built.md`, `docs/as-built.md`. |
| Retirement-Statement-Ingestion | **COMPLETE**, monthly task wired 2026-07-09 | `retirement-fund.md` manual ledger retired (2026-07-08) — replaced by deterministic (no-LLM) parsing of Sharesies KiwiSaver PDF statements. New `monitoring/portfolio_statement_ingest.py`: regex label/value extraction (stable across 13 real statements 2023–2026), idempotent by `(document_type, period_end)` + source modified-ts, writes `semantic:portfolio:sharesies:{quarterly\|annual}:{period_end}`. Entry point `/portfolio ingest` (`_run_portfolio_harness`, `category=="ingest"`) — synchronous, ~13 files in seconds. `portfolio_analysis_harness.py`'s `resolve_category()` gets `_resolve_retirement_from_statements()` for `slug=="retirement-fund"`: builds `AssetSpec`s from ingested statements + a Director-authored `semantic:portfolio:retirement:target_allocation` record (target %/rebalance bands by holding name), `_STAGED_EXIT_PLAN` dict encodes the agreed staged exit for 4 pre-redesign holdings (AIQ full exit; RKLB/Milford/Infratil 25%-per-swap), every spec `extra["locked"]=True` (KiwiSaver — internal swaps only, no cash exit). `run_portfolio_watcher_scan()` gained a `category=="retirement"` contribution-drift check (portfolio-level, not per-asset); crypto-only extras (Correlation Analyst, stress tests) already self-gated on `category`/`slug=="crypto"` so no code changes needed there. Known gaps: cost basis not in statements (`total_cost_basis_nzd`=0.0 for this category, deferred per Director); 4 ticker-mapped holdings (FPH/FSF/GNE/NPF) have no target_allocation entry so can never fire DRIFT_ALERT — flagged, not fixed. 13 statements ingested + verified live. **2026-07-09**: wired into the monthly portfolio task only (not weekly — statement data updates quarterly at best, Director's call) via a second step on the existing Director-approved monthly task (`task_id 571e259a...`); live `procedural` record patched directly (`set_payload`) since the seed function's idempotency check would otherwise have skipped the already-active task on next restart. Full detail: `docs/as-built.md` 2026-07-08/09 entries. |
| Rex-trics | **CHECK 1/2 COMPLETE** (built uncommitted, undocumented until now) **+ LOG PHASE 1 COMPLETE 2026-07-10** **+ DASHBOARD PANEL COMPLETE 2026-07-10** **+ DEPLOYED + TELEGRAM COMMAND COMPLETE 2026-07-11** **+ DETECTOR ACCURACY FIX 2026-07-19** | Deterministic hallucination/performance flag counters + per-interaction retention, `cognition/rextrics.py`. **Check 1** (`orchestrator_evaluate`/PASS4, `cognition/engine.py` ~1113-1148): grounds the LLM-drafted `result_for_translator.success` against `execution_confirmed` (deterministic, HTTP-status-derived — see 2026-07-19 fix below; originally compared against the specialist's own claimed success, which was correlated-by-construction and essentially never fired) — deterministic override on mismatch, flags `execution_mismatch`. **Check 2** (`translator_pass`/PASS5, ~1267-1274 + `_check_unverified_claims()`): strips memory-claim-shaped sentences ("I remember...", "per our conversation...") unbacked by any grounding evidence in the result, flags `unverified_claim`; the deterministic number-fabrication catch (`_check_fabrication()`) also routes into this same counter as of 2026-07-19. **Rex-trics Log** (Phase 1, additive, no changes to Check 1/2): one per-interaction record per `translator_pass()` call — `ts`/`intent_classified`/`domain_routed`/`outcome`(success\|fail\|fallback)/`reason`/`latency_ms` — fired from the same chokepoint `record_response()` already used, so all 22 confirmed call sites (main 5-pass flow, PASS1 timeout/exception, governance/security blocks, memory/research short-circuits, all `/command` harness completions, `main.py`'s wallet-event webhook) are covered with zero new call sites. `outcome="fallback"` = `routing_source` is `llm_pass1` or `fallback_map` (classification didn't hit a deterministic `_quick_classify` fast path) — checked before success/fail, since a fallback-routed request can still execute successfully. success/fail otherwise via new `_normalize_outcome()` — checks both the `status`(ok/error) and `success`(True/False) return conventions coexisting across `_dispatch_inner`'s ~36 domains; unrecognised/missing shape → `fail`/`unnormalized_response` (deliberately not guessed). `latency_ms` is new instrumentation (delta from `handle_chat()` entry to `translator_pass()` return) — repurposes a pre-existing but genuinely dead local timestamp (`_t_total`, execution/engine.py, never previously read). First cut stored it as a shared `self._request_t0` instance attribute (mirroring `_had_queue_wait`); live smoke-testing 2026-07-10 found this unsafe under overlapping `handle_chat()` calls on the same `ExecutionEngine` instance (one request's t0 can be clobbered by another's before the first reads it back) — corrected to a `handle_chat()`-local `_req_t0`, threaded explicitly as a `request_t0` parameter into all 6 `/command` harness methods it calls (`_run_install_harness`, `_run_skills_browse`, `_run_selfimprove_harness`, `_run_devcheck_harness`, `_run_signbtc_harness`, `_run_portfolio_harness`), each with exactly one call site confirmed before the change. No shared mutable state left on `self` for this. Storage split, no change to existing shape: `meta:rextrics:daily:{date}:counters` (aggregates, untouched) + new `meta:rextrics:daily:{date}:entries` (appended list, same read-modify-write upsert / accepted lost-append risk as the rest of META — no new atomic primitive). `/rextrics` (`_quick_classify` fast-path, `_DIAGNOSTIC_INTENTS`, LOW tier, `governance/engine.py` `domain=='rextrics'`) unchanged — `get_weekly_report()` extended, not replaced: still returns the this-week/prior-week counter trend deltas, now merged with read-time-only Log stats (success/fail/fallback rate, active days out of 7, median/p90 latency, top 3 fail reasons) computed fresh from `:entries` on every read, nothing pre-aggregated. `component_registry.py` module entry added same session (`module-rextrics`, corrected the pre-existing-stale "69 components" count to the actual 72). **Dashboard panel** (same session, 2026-07-10): portal §07 "Live Log Stream" (broken SSE panel) replaced with a Rex-trics weekly summary card reading `get_weekly_report()` — success/fail/fallback rate, active days X/7, median/p90 latency, top 3 fail reasons, this-week-vs-prior-week trend deltas. New `cognition.rextrics.write_last_report_cache()` (same META upsert pattern, `meta:rextrics:last_report`) called from `execution/engine.py`'s existing `domain=="rextrics"` dispatch on every `/rextrics` invocation; new `GET /rextrics/summary` in `portal.py` reads it back via the existing `qdrant.retrieve_by_key()`. Static snapshot by design — Director's call was to wire the fetch into the normal 5s `doRefresh()` cycle (not a load-once fetch) since the Director refreshes the underlying data manually via `/rextrics` in Telegram, not by reloading the dashboard. SSE log-stream code binned outright per Director ("just bin it" — SSH covers live logs): `GET /logs/stream`, `_stream_container_logs`/`_sse_log_generator`, all `initSSE`/`sseToggle`/`sseFilterToggle`/`renderSSELog` JS, the dead `renderAudit()` function, the `log_containers`/`log_tail_each_container`/`log_heartbeat_s` config knobs, and the documentation-only `named_commands.log_tail_sovereign` block in `docker-policy.yaml` (the underlying `GET:/containers/*/logs` permission itself was kept — shared with Rex's own `docker_logs` intent). **Deploy + Telegram command** (2026-07-11): rebuild/restart of `sovereign-core` surfaced a real bug the diff review missed — nginx's portal server block only proxies an explicit allowlist regex of paths, and `/rextrics/summary` wasn't in it (403, never reached sovereign-core); fixed by adding it and dropping the now-dead `/logs/stream` block from `nginx/nextcloud.conf` in the same edit. That edit then exposed a second gotcha: `nginx/nextcloud.conf` is a **single-file** bind mount, so the edit tool's write-via-rename changed the inode and `nginx -s reload` kept serving the stale pre-edit content — needed `docker compose restart nginx` to re-establish the mount (documented as a third nginx failure mode in Claude Code's durable memory, `feedback_nginx_ops.md`, alongside the two from 2026-07-05). Separately, `/rextrics` turned out to have no real Telegram slash command at all — only the NL keyword match — so a literal `/rextrics` went nowhere (Telegram tags anything starting with `/word` as a command entity, and the gateway's catch-all excludes those regardless of whether a handler exists). Added `handle_rextrics()` + `CommandHandler("rextrics", ...)` in `gateway/main.py` (mirrors `handle_selfimprove`, no `_harness_cmd` — rextrics has no session state, it's a single deterministic read); `gateway` rebuilt (this one *is* baked into its image, unlike sovereign-core's bind-mounted source) and restarted. Full accurate BotFather `/setcommands` block produced for all 14 working commands. Two post-deploy UI tweaks: a second always-visible refresh hint added directly to the §07 section header bar (the original dynamic note was only 9px, easy to miss), and that note's font bumped 9px→11px. Live-verified end-to-end after a real `/rextrics` Telegram run: `GET /rextrics/summary` returned a fully populated report. Full detail: `docs/as-built.md`'s 2026-07-11 entry. **Detector accuracy fix (2026-07-19)**: a real week reporting 0/0 on both counters (twice running) prompted a check on whether Check 1/2 were wired correctly — plumbing confirmed fine, but two real detection gaps found. Fix 1: Check 1 rewired to compare against `execution_confirmed` (new param threaded through `orchestrator_evaluate()` from the already-in-scope `execution/engine.py:3894` value) instead of `specialist_inbound_result.success`, since PASS 4's draft defaults from that same field and the old comparison was correlated by construction. Fix 2: the fabrication catch in `translator_pass()` now also calls `record_flag(..., "unverified_claim")` before its early return — previously a blocked fabrication was invisible to the stat despite being exactly what it's meant to track. Verified deterministically (mocked LLM calls, real comparison/override logic) rather than waiting for a live hallucination — both assertions passed. Full detail: `docs/as-built.md`'s 2026-07-19 entry. |

| Daily-Technicals-BTCD | **CODE COMPLETE, UNCOMMITTED (2026-08-04)** | Daily-resolution sibling to the existing weekly/monthly RSI/MACD path — new `monitoring/technicals_daily_harness.py`. Build 1: daily price snapshots (Yahoo, self-healing 5-day rolling window) + ETH/BTC leg-derived ratio + BTC.D dominance (CoinGecko `/global`, resolved from a new MIP semantic entry — first real MIP-endpoint consumer in the codebase) + dominance source-health alert. A live-tested unbounded-concurrency bug (35-way `asyncio.gather` against Yahoo → 75-80% `httpx.ReadTimeout` failure) corrected the plan's own "safe here, pure HTTP" assumption; fixed with `asyncio.Semaphore(6)`, matching the `sovereign-browser` skill's own precedent. Build 2: `_gather_technicals_daily`/`DailyTechnicalData`, compute-only (≥15-sample RSI gate, ≥35-sample MACD gate), not yet wired into alerting. Build 3: BTC.D alt-season trigger (`meta:dominance_trigger:state`), notify-then-persist ordering (Telegram send confirmed before the `fired_60`/`fired_55` flag persists, so a crash/failed-send retries next run rather than silently swallowing the signal) — required an opt-in `raise_on_failure` param on the shared `_notify_telegram()` helper (~15 existing callers, default behaviour unchanged). Fully validated via a synthetic 61→59→58→62 sequence + forced-send-failure + retry scenarios (real dominance never crossed a threshold live this session). Build 4: shadow-compare — resamples the accumulated daily series to weekly and diffs against the existing weekly path's own `technical_snapshot` record (read back, never re-fetched); deliberately sequential, not concurrency-bounded (Qdrant-only, no external HTTP fan-out). Build 5: completeness reminder — ~48-month/1440-day gapless-history check, one-off dedup-guarded PROSPECTIVE proposal (no data-condition schedule type exists, so checked inline in the daily task itself, matching `self_improvement.py`/`cognition/subjects.py` precedent) — the Yahoo-cutover trigger, cannot realistically fire for ~4 years. Scheduler + governance: `seed_daily_technicals_task()` (07:10 NZST daily, `execution_timeout_s: 900` sized from a real 383.1s measurement), new `technicals_daily` governance domain (gates on `memory_write`, not `browser_search` — direct httpx, not nanobot), `_dispatch_inner` entry. A cron-collision idempotency bug was found and fixed live on first deploy (the seed silently no-op'd against an unrelated pre-existing task sharing the same `"0 19 * * *"` slot) — fixed by offsetting to `"10 19 * * *"` rather than touching the shared title-OR-cron idempotency check used by 5+ other seed functions. Director-approved live via Telegram same session. Companion work: `AlphaVantageAdapter` (dead code, zero live call sites) deleted in full — adapter file, routing branches, governance.json registry entry — `_gather_alpha_vantage()`'s independent fundamentals fetch kept. Full detail, all validation evidence, and forward-looking open threads (a flagged-but-unbuilt embed-call queue gap, CoinGecko client consolidation, full daily signal-band evaluation): `docs/daily-technicals-as-built.md`. |
...
| Tax-Report-Harness (`/do_tax`) | `_tax_report_harness_checkpoint` | query → ingest → create → notify → clear | 3-turn human-in-the-loop; generates income, expenses, and disposals CSVs; FIFO cross-year lot pool |
| Receipt-Capture-Harness (`/receipt`) | `_receipt_capture_checkpoint` | capture → confirm (or corrections) | Gemini Vision; GEMINI_API_KEY required; WORKSPACE_INTERNAL minimum; 60s vision timeout; TaxEvent tag tax:expense; photo uploaded to /Digiant/Tax/FY{year}/ post-confirm (non-blocking); multi-currency: checkpoint stores `amount_original` (raw extracted) + `amount_nzd` (NZD decimal string, empty if unknown) + `currency`; foreign-currency receipts set nzd_value=None until Director replies "nzd XX.XX" |
...
| Learn-On-Demand (`/learn`) | none (no sentinel, always re-runnable) | text→keywords→doc_array→chunk(cap 6)→confidence loop→summary | One-shot; URL fetched via browser if detected; surfaces already-known entries + written count to Director |
...
| `/do_tax [year]` | Tax-Report-Harness | Generate NZ tax report for FY; 3-turn human-in-the-loop; produces income, expenses, and disposals CSVs with FIFO gain/loss |
| `/receipt` | Receipt-Capture-Harness | Send photo with /receipt caption → Gemini Vision extracts fields → Director confirms → TaxEvent written to semantic memory; foreign currency: prompts for NZD equivalent before storing |
| `/learn <text or URL>` | Learn-On-Demand | Integrate inline text or fetched URL into semantic memory; returns summary of already-known entries + what was written |
