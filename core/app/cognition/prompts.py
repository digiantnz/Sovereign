...
devops_agent:
  ...
  self_improve_proposals   — list pending self-improvement proposals generated by the SI harness after a /devcheck run; use when Director asks what was found or what proposals are waiting (LOW)
  self_improve_baseline    — show the SI harness baseline report: the known-good state snapshot used to detect regressions (LOW)
  self_improve_dismiss     — dismiss and clear all pending SI proposals without applying them; use when Director says to ignore findings (LOW)
...
business_agent:
  ...
  list_files_recursive  — list every file in a Nextcloud folder and all subfolders; use when Director wants a full tree or says "all files in" (LOW; target: folder path)
  read_files_recursive  — read the contents of every file in a Nextcloud folder recursively; use for "read everything in", "ingest folder", "read all files in" (LOW; target: folder path)
  create_note           — create a new note in Nextcloud Notes (MID — requires confirmation; target: note title; body comes from the rest of the input)
  read_note             — read the body of a specific Nextcloud note; use when Director names a note or gives an ID (LOW; target: note ID or title)
  update_note           — update/overwrite an existing Nextcloud note's content (MID; target: note ID or title)
  delete_note           — permanently delete a Nextcloud note (HIGH — requires double confirmation; target: note ID or title)
  list_notes            — list all notes in Nextcloud Notes with titles and IDs (LOW)
  ncfs_list             — list files and folders in a Nextcloud directory via WebDAV; use for "list nextcloud", "what's in downloads", "show the downloads folder" (LOW; target: folder path, default "/")
  ncfs_list_recursive   — recursively list the full tree of a Nextcloud directory; use when Director says "full tree", "entire nextcloud", "everything on nextcloud" (LOW; target: folder path)
  ncfs_read             — read a specific file's content from Nextcloud via WebDAV (LOW; target: full file path e.g. "/downloads/report.md")
  ncfs_mkdir            — create a new folder on Nextcloud WebDAV; prefer this over create_folder when Director says "on nextcloud" (MID; target: full folder path)
  ncfs_delete           — delete a file or folder from Nextcloud WebDAV (HIGH — requires confirmation; target: full path)
  ncfs_move             — move or rename a file on Nextcloud; use for "move file", "rename file", "move it to" (MID; provide source path as target and destination in the input)
  ncfs_copy             — copy a file to another Nextcloud location; use for "copy file", "duplicate file", "copy to" (MID; source path as target, destination in input)
  ncfs_tag              — tag a Nextcloud file with a label; use for "tag the file", "add tag", "label the file" (LOW; target: file path; include tag name in input)
  ncfs_untag            — remove a tag from a Nextcloud file; use for "untag", "remove tag" (LOW; target: file path)
  ncfs_search           — search Nextcloud for files by name or content; use for "search nextcloud for", "find files named", "find file named" (LOW; target: search query)
  ingest_status         — check whether a specific Nextcloud file has already been reviewed and tagged as ingested (LOW; target: file path)
  ingest_file           — ingest a single Nextcloud file into sovereign semantic memory (LOW; target: file path)
  ingest_folder         — ingest all files in a Nextcloud folder into sovereign memory (LOW; target: folder path)
  portfolio_analysis_save  — save the current in-session portfolio analysis report to Nextcloud; use when Director says "save portfolio" or "save the portfolio" (MID)
...
- "improvement proposals", "pending proposals", "list proposals", "show proposals", "si proposals", "what findings", "what anomalies" → devops_agent, intent=self_improve_proposals (LOW)
- "show baseline", "baseline report", "si baseline" → devops_agent, intent=self_improve_baseline (LOW)
- "dismiss proposals", "clear proposals", "dismiss si proposals", "dismiss improvement proposals" → devops_agent, intent=self_improve_dismiss (LOW)
...
- "update note", "edit note", "change note", "modify note" → business_agent, intent=update_note (MID)
- "delete note", "remove note" → business_agent, intent=delete_note (HIGH)
- "list nextcloud", "what's on nextcloud", "what's in downloads", "list the downloads", "show downloads" → business_agent, intent=ncfs_list (LOW; target=folder path)
- "full tree", "entire nextcloud", "everything on nextcloud", "recursive nextcloud" → business_agent, intent=ncfs_list_recursive (LOW)
- "read from nextcloud", "fetch from nextcloud", "get the file from nextcloud", "read nextcloud file" → business_agent, intent=ncfs_read (LOW; target=file path)
- "create folder on nextcloud", "mkdir on nextcloud", "make folder on nextcloud" → business_agent, intent=ncfs_mkdir (MID)
- "delete from nextcloud", "remove from nextcloud", "delete the nextcloud file" → business_agent, intent=ncfs_delete (HIGH)
- "move the file", "move file to", "rename file" [on Nextcloud] → business_agent, intent=ncfs_move (MID)
- "copy the file", "copy file to", "duplicate file" [on Nextcloud] → business_agent, intent=ncfs_copy (MID)
- "tag the file", "add tag", "label the file", "untag" [Nextcloud] → business_agent, intent=ncfs_tag or ncfs_untag (LOW)
- "search nextcloud for", "find files named", "search for files" → business_agent, intent=ncfs_search (LOW; target=search query)
- "ingest status", "has this been reviewed", "check if reviewed" → memory_agent, intent=ingest_status (LOW)
- "ingest", "review and remember", "add to memory", "read and remember" + file path → memory_agent, intent=ingest_file or ingest_folder (LOW)
- "list all files", "list files recursively", "all files in [folder]" → business_agent, intent=list_files_recursive (LOW; target=folder path)
- "read all files", "read the folder", "read everything in" → business_agent, intent=read_files_recursive (LOW; target=folder path)
- "save portfolio", "save the portfolio", "portfolio save" → business_agent, intent=portfolio_analysis_save (MID)
- "clear portfolio", "clear portfolio analysis", "discard portfolio" → business_agent, intent=portfolio_analysis_clear (LOW)
